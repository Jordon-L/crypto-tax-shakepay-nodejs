  ...
  try {
    ...
    if (unsoldHoldings[year - 1]) {
      ...
      filteredTransactions.unshift(
        previousYearHoldingsETH,
        previousYearHoldingsBTC
      );
    }
    let columns = [
...
function filterByYear(df, year) {
  ...
  if (isNaN(parsed)) {
    ...
    return df.filter(
      (entry) =>
        entry["Date"].getTime() >= startOfYear &&
...
function peerTransfer(row) {
  ...
  if (row["Direction"] === "credit") {
    ...
    if (row["Spot Rate"] === "") {
      ...
      let numerator = Decimal.add(
        Decimal.mul(currentAvg, totalCreditCurrency),
        Decimal.mul(row["Spot Rate"], credit)
      );
...
function walletReceive(row) {
  ...
  if (
    row["Source / Destination"].toLowerCase() === shakepayWallet.toLowerCase
  ) {
    event = "Internal transfer";
...
function walletSend(row) {
  ...
  setCurrencyTotals(
    debitCurrency,
    Decimal.sub(totalDebitCurrency, Decimal.add(debit, row["fees"]))
  );
  let salePrice = row["Spot Rate"];
  let costToObtain = 0;
  let credit = Decimal.mul(salePrice, debit);
  ...
  if (
    shakepayWallet.toLowerCase() === address.toLowerCase() ||
...
function calculateTax(table) {
  ...
  capitalLossRow.forEach((row) => {
    ...
    for (let i = 0; i < purchases.length; i++) {
      ...
      if (
        (buyDate > before || buyDate < after) &&
        purchase["Credit Currency"] === row["Debit Currency"]
      ) {
  ...
  capitalGainsTax = capitalGainsTax.filter(
    (row, index) => capitalRow[index]["Event"] !== "Superficial Loss"
  );
...
async function mergeEtherScan(shakepayData, address) {
  try {
    ...
    let etherScanData = await ethScan.getEthTransactions_ShakepayFormat(
      address,
      "ethereum",
      "CAD"
    );
