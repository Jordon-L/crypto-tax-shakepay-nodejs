    ...
    content = (
      <Grid container className={classes.grid} alignItems="stretch">
            ...
            >
              <CardContent className={classes.inputCard}>
                ...
                <a
                  ...
                  target="_blank"
                  className={classes.helpfulLinks}
                >
                  How to get Shakepay CSV
                </a>
              </CardContent>
              <input
                type="file"
          ...
          <Card className={classes.cardHeight}>
            <CardContent>
              ...
              <CardActions>
                <TextField
                  fullWidth
                  label="Ethereum Wallet Addresses"
                  variant="outlined"
                  value={wallet}
              ...
              <CardActions>
                <TextField
                  error={errorEth}
                  fullWidth
                  label="Shakepay Ethereum Wallet Address"
                  variant="outlined"
                  value={shakepayWallet}
                  onChange={(event) => setShakepayWallet(event.target.value)}
                />
              </CardActions>
            </CardContent>
          </Card>
        ...
        <Grid item xs={6} className={classes.gridItemRight}>
          <Card className={classes.cardHeight}>
            <CardContent>
              This is a tax calculator for the crypto trading platform,
              Shakepay. Upload your csv file from Shakepay. Integration with
              Ethereum blockchain is available under Advance Options.
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12}>
      ...
      <Grid container className={classes.grid}>
        <Grid item xs={12} className={classes.taxInformation}>
          <Button
            ...
            variant="contained"
            color="primary"
            onClick={() => Reset(setTaxInfo, setTotals)}
          >
            Go Back
          </Button>
          <Button
          ...
          <Card className={classes.taxInformation}>
            <CardContent>
              <Accordion disabled className={classes.disabledAccordion}>
                ...
                >
                  <Typography>Income: {taxInfo.incomeGain}</Typography>
                </AccordionSummary>
              </Accordion>
              <Accordion>
                <AccordionSummary
                  ...
                  id="panel2a-header"
                >
                  <Typography>Capital gain: {taxInfo.capitalGain}</Typography>
                </AccordionSummary>
                <AccordionDetails>
                  <Typography className={classes.moreDetail}>
                    BTC
                    <Typography>Total BTC: {taxInfo.totalNumberBTC}</Typography>
                    <Typography>
                      Total Sale: {taxInfo.totalSalePriceBTC}
                    </Typography>
                    <Typography>Total Cost: {taxInfo.totalCostBTC}</Typography>
                    <Typography>Total Fees: {taxInfo.totalFeesBTC}</Typography>
                    <Typography>Total Gain: {taxInfo.totalGainsBTC}</Typography>
                  </Typography>
                  <Typography className={classes.moreDetail}>
                    ETH
                    <Typography>Total ETH: {taxInfo.totalNumberETH}</Typography>
                    <Typography>
                      Total Sale: {taxInfo.totalSalePriceETH}
                    </Typography>
                    <Typography>Total Cost: {taxInfo.totalCostETH}</Typography>
                    <Typography>Total Fees: {taxInfo.totalFeesETH}</Typography>
                    <Typography>Total Gain: {taxInfo.totalGainsETH}</Typography>
                  </Typography>
                </AccordionDetails>
              </Accordion>
            </CardContent>
...
function Reset(setTaxInfo, setTotals) {
  setTaxInfo({});
  setTotals({});
}
function Upload(
  ...
  if (selectedFile == null || selectedDate == null) {
    ...
    (wallet != null && shakepayWallet == null) ||
    (wallet != null && shakepayWallet == "")
  ) {
    setErrorEth(true);
    setLoading(false);
  } else {
